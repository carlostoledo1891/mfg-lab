...
- A third copy of the rational arithmetic, hiding in a downstream battery,
  found by the single-source scan within a minute of that gate existing.

## Guarantees about copies

The repository this toolkit is developed in runs a single-source scan over its
whole working tree for the *fingerprints* of a reimplementation, failing on any
file carrying them that is not the toolkit or a declared, byte-identical
vendored copy. It is a text scan on purpose: a second copy is created by
pasting, not by importing.

**That scan is not part of this published copy**, because its register names
unpublished work. So this distribution carries no single-source gate of its
own: a second copy of the interval or rational library could be added here and
nothing in this tree would detect it. The guarantee holds where the toolkit is
developed; this copy's self-check is absent until a public-scoped register
exists.
...
```bash
make check     # 26 checks + 4 falsifiers, plus the single-source gate
```
