...
/* M5b, 2026-07-28: `'../../../eqcert/...'` before the instruments moved to lab/ at the tree
   root. That count was right in the monorepo (academic/mfg-lab/lab -> root) and WRONG in the
   exported tree (mfg-lab/lab -> one level ABOVE the export root), where this file also ships —
   a break M4 introduced when eqcert moved to the root, and one no gate in this repo could see,
   because every local run takes the monorepo branch. With lab/ at the root of BOTH trees a
   single `..` is now correct in both, so the move retires the two-tree hazard rather than
   moving it. */
const { proved, refused } = require('../core/interval/certificate.js');
