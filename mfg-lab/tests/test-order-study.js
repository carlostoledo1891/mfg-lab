/* test-order-study.js — battery for the Lab's first feature: a convergence
   study that refuses to lie (lab/order-study.js).
...
/* M5b, 2026-07-28 — the instruments moved to `lab/` at the TREE ROOT. This battery SHIPS, and at
   that time mfg-lab/tests sat THREE levels under the monorepo root (academic/mfg-lab/tests) and TWO
   under the export root (mfg-lab/tests), so no fixed count was correct in both trees and the two
   were ENUMERATED, nearest-existing wins.

   2026-07-29 — THE ENUMERATION IS NO LONGER NEEDED, and this is the point of the move that retired
   it: mfg-lab left academic/ for the repository root, so this file now sits TWO levels under the
   root in BOTH trees and one ascent count is correct in both. The dead three-segment candidate is
   removed rather than left as a harmless-looking fallback — a candidate that can never match is
   indistinguishable from one that stopped matching, and check-paths.js reports it as escaping the
   repository, which is exactly what it was doing.

   The loud failure below is KEPT. Reducing two candidates to one removes the ambiguity, not the
   need to say so when the address is wrong. */
const LAB = require('./_root.js').rootPath('lab');   /* marker-resolved: survives any depth */
if (!LAB) {
  console.error('test-order-study.js: the lab/ instruments were not found from ' + __dirname);
  console.error('  expected lab/contract.js at the tree root (M5b moved them there).');
  process.exit(2);
}
...
for (const f of ['contract.js', 'order-study.js'])
  console.log('   ' + path.join('lab', f) + '  sha256:' + sha(path.join(LAB, f)));
