/* test-failure-map.js — battery for the Lab's second feature: where does your
   solver stop working (lab/failure-map.js).
...
/* M5b, 2026-07-28 — the instruments moved to `lab/` at the TREE ROOT. This battery SHIPS, and
   mfg-lab/tests sits three levels under the monorepo root but two under the export root, so no
   fixed `..` count serves both; the trees are enumerated. See test-order-study.js. */
const LAB = [require('./_root.js').rootPath('lab')]         /* export:   mfg-lab/tests */
            .find(d => fs.existsSync(path.join(d, 'failure-map.js')));
if (!LAB) {
  console.error('test-failure-map.js: the lab/ instruments were not found from ' + __dirname);
  console.error('  expected lab/failure-map.js at the tree root (M5b moved them there).');
  process.exit(2);
}
...
console.log('   lab/failure-map.js  sha256:' + sha(path.join(LAB, 'failure-map.js')));
