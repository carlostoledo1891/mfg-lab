/* _root.js — find the TREE ROOT without counting `..`.
   ...
   that live at the tree root in every layout: `eqcert/` (the shared toolkit), `lab/` (the JS
   ...
   contains `eqcert/` — true in the monorepo, true in the shared export, and the one marker that
...
/* The nearest ancestor containing `eqcert/`. Marker, not a count. */
function treeRoot(from) {
  const start = from || __dirname;
  for (let d = start; ; d = path.dirname(d)) {
    if (fs.existsSync(path.join(d, 'eqcert'))) return d;
    if (path.dirname(d) === d) break;
  }
  throw new Error('mfg-lab/tests/_root.js: no directory above ' + start + ' contains eqcert/ — ' +
