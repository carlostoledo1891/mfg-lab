/* test-index.js — the gate for the LANDING PAGE, and for the shape of it.
   ...
   WHAT IS CHECKED, AND WHY EACH ONE EXISTS
     ...
     I1  every site-absolute href on the landing page resolves THROUGH THE
         ...
         these pages are served from rewrites, so `/mfg-cap` is right and
         ...
         `/mfg-cap` and `/sin-mfg` are M7b's permanent redirects into `/papers/`;
   ...
   public tree resolves every route directly. Then, one mutation per copy:
     ...
     exit 1  DELETE papers/mfg-cap.html, the file the `/mfg-cap` 301 lands on.
...
/* AMENDED 2026-08-08 (REBUILD_PLAN phase 2). The lab moved from research/mfg-lab to site/mfg-lab
   — its route — and the artifact from mfg-lab.html to index.html, because /mfg-lab was the last
   address on this site served by a Vercel rewrite instead of by a file sitting where it is
   addressed. Under cleanUrls, mfg-lab/index.html answers /mfg-lab directly and the rewrite is
   retired. The two-shape lookup stays: this battery ships, and in the export the lab is at the
   flat mfg-lab/. */
const LABDIR = [path.join(ROOT, 'site', 'mfg-lab'), path.join(ROOT, 'mfg-lab')]
                 .find(d => fs.existsSync(d)) || path.join(ROOT, 'mfg-lab');
const ART = path.join(LABDIR, 'index.html');
...
if (!fs.existsSync(ART)) {
  ...
  console.error('   /mfg-lab landing page; if it moved, re-point ART, and if it');
...
/* I0 REPHRASED 2026-08-08, and the rephrasing is the point rather than a repair. It read
   ...
   the real property, which is that ONE page is the hub: the artifact answers /mfg-lab itself and
...
ok(rootPages.length === 1 && rootPages[0] === 'index.html',
...
/* A DEPLOYED FILE PATH (`papers/mfg-cap.html`) -> THE BYTES ON DISK.
   ...
   `<academic>/mfg-cap/mfg-cap.html` stopped existing and I1/I4 reported four pages MISSING
...
/* I1b — reachability is a property of the DESTINATION, not of the route string.
   ...
   onepager became the homepage, `/` -> onepager and the self-route became /mfg-lab,
   so the string test started failing on /mfg-lab and on /onepager — an alias of `/`,
...
const SELFFILE = path.relative(ROOT, ART).split(path.sep).join('/');    // e.g. mfg-lab/index.html
/* The two tree shapes this file already documents at LABDIR bite here too: in the monorepo
   SELFFILE is `site/mfg-lab/index.html`, while the route table resolves /mfg-lab to the
   EXPORT path `mfg-lab/index.html`. String equality therefore reported the artifact as
   orphaned by itself in one tree and not the other — which surfaced on 2026-08-03 when the
   footer site map was rebuilt and stopped carrying a /mfg-lab self-link that had been masking
   it. Same file, two spellings; compare it as one. Teeth are unchanged: a destination nothing
   links to, and which is not this artifact, still fails. */
/* The prefix stripped here is the PUBLISHED-TREE prefix, not one project's parent. It was
   `research/` until 2026-08-08; phase 2 of the rebuild moved every page under site/, which
   publishes at the export ROOT, so the monorepo spelling gained a `site/` that the deployed
   spelling does not have. Both prefixes are stripped rather than just the current one, so this
   line stays true in a tree where either move is what happened. */
...
for (const [src] of ROUTES) {
  const s = serve(src);
  /* A route that serves no file is I4's finding, which carries the repair instruction.
     Asserting it here too would double-report one defect, and passing it here would be a
...
/* WHO I4b IS ADDRESSED TO — "this page raises installing THE DISTRIBUTION".
   ...
   scope and the substring immediately misfired on it: site/index.html says the verifier is
...
/* ------------------------- I5 · prose guards — MOVED 2026-07-30, not deleted
   ...
   prose now lives on the site pages, and the check is re-homed to site/tests/test-layout-site.py
