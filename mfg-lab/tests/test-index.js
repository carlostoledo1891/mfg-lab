...
const LABDIR = [path.join(ROOT, 'research', 'mfg-lab'), path.join(ROOT, 'mfg-lab')]
...
if (!fs.existsSync(ART)) {
  ...
  console.error('   This is a MISSING FILE, not a failed assertion. The landing page is a');
  console.error('   route of mfg-lab.html; if that artifact moved, re-point ART, and if it');
...
const SELFFILE = path.relative(ROOT, ART).split(path.sep).join('/');    // e.g. mfg-lab/mfg-lab.html
/* The two tree shapes this file already documents at LABDIR bite here too: in the monorepo
   SELFFILE is `research/mfg-lab/mfg-lab.html`, while the route table resolves /mfg-lab to the
   EXPORT path `mfg-lab/mfg-lab.html`. String equality therefore reported the artifact as
...
const isSelf = (f) => f === SELFFILE || f === SELFFILE.replace(/^research\//, '');
...
/* WHAT WENT WRONG HERE, because the shape of the bug is the reason for the shape
   ...
   The old I4 read a hardcoded list, `['mfg-lab.html', 'lab.html']`, and opened
