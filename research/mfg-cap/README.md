...
> What is, **to our knowledge**, the first rigorous computer-assisted enclosure of a solution to a mean-field
...
same norm. **Disjoint enclosures at identical parameters exhibit at least two
distinct solutions** — a statement about the PDE, not about a solver.

> **A note on the words.** *proved* is rung 5 of this project's evidence ladder
> and *certified* is rung 4; rung 5 needs a proof document
> with a named human reviewer and a signature and has **no agent path**, rung 4
> needs a ledger certificate record — and the ledger holds **no record for this
> system at all** (every claim and certificate on file is about the congestion
> game, at other parameters). Neither ladder word is used below as a **status**.
> The verb *certify* still appears for the **mechanism**, which is the product
> and is not a status.

<!-- EXPORT BOUNDARY, 2026-07-29. Two file pointers were removed from the note
     above. Both resolved only in the upstream working tree; neither is on any
     export list, so in this tree they were dangling references and the export
     scanner flagged them. One was also an internal record name in reader-facing
     text. The note's substance is unchanged — it states inline what rungs 4 and 5
     require. Do not restore those pointers: a shipping file may not cite a file
     that does not ship. (This comment deliberately names no upstream path.) -->
...
|---|---|---|---|---|---|
| −11 | −0.2445 | 3.20e−13 | 6.71e−15 | 6.70 | ≥2 solutions enclosed |
| −12 | −0.3421 | 3.38e−13 | 6.84e−15 | 10.79 | ≥2 solutions enclosed |
| −16 | −0.6257 | 1.54e−09 | 1.17e−14 | 27.28 | ≥2 solutions enclosed |
| −24 | −1.0885 | 1.96e−05 | 1.68e−14 | 70.01 | ≥2 solutions enclosed |
...
The mathematics moved to `core/` on 2026-08-08 — one implementation, many consumers — so this
unit's kernels are no longer beside this file. `tools/build-artifact.js` reads them from there and
the page is generated FROM them, which is the property that matters and is unchanged.


```
core/interval/interval.js  outward-rounded interval arithmetic; the rigor model
core/mfg/mfg1d.js          the model in Fourier + spectral Newton (floats only)
core/mfg/validate.js       the proof: Y0, Z1, Z2, radii polynomial, positivity
tests/test-cap.js    17 checks + 6 falsifiers
