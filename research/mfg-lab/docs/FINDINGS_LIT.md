...
| interior point >30 min vs HRF <1 s | "the conventional solver needed more than 30 minutes"; multi-pop "converged in less than 0.2 seconds" | **PREPRINT ONLY — verdict DOWNGRADED from OK, 2026-07-28.** Both quotes are accurate transcriptions of arXiv:2504.16028**v1** §V, and this row was right about the bytes it read. But **the authors removed the claim before print**: the PUBLISHED paper this repo cites (AIMS Math. 11(5):15143–15162, DOI 10.3934/math.2026623), Table 2, **same Fig. 1a Scenario-1 instance**, says the opposite — *"the uniform NLP solver is the fastest method overall"*, 0.00053 s, with HRF slowest at 0.05–0.07 s. `research/wardrop-repro/docs/PAPER_FACTS.md §9.4` transcribes the published version and is the record in force. The two records disagreed for months and **both were correct, about different versions.** Since the citation names the DOI, **this claim may not be attributed to "the paper"** — only to a superseded preprint draft. It was removed from `mfg-lab.html` on 2026-07-28. |
...
## Method
M1 kernel extracted brace-balanced from research/mfg-lab/mfg-lab.html into `kernel_param.js`
...
Battery: `research/mfg-lab/tests/test-invariant.js` (in `make check`; extracts the MGG
...
1. **A prior planning note recorded that MF-OMO "bounds exploitability by the
   ...
   **RULES: the abstract is not the paper — never conclude a body-level claim
   ...
   here.**
2. **"Uncited" was itself half wrong.** research/mfg-lab/mfg-lab.html already named `MF-OMO,
