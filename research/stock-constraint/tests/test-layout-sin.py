...
# ONE LAYOUT, ONE ARTIFACT — see the note in test-sin.js. The candidate list existed because the
# note had a different address in the monorepo and in the export; the structure migration makes the
# two the same string, so a list whose later entries can never be chosen would only mislead.
_SIN_CANDIDATES = [HERE.parent / 'stock-constraint.html']
