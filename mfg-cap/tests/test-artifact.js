...
const SOURCES = [['core/interval/interval.js', EQ],
                 ['core/mfg/mfg1d.js', upward(ROOT, path.join('core', 'mfg', 'mfg1d.js'))],
                 ['core/mfg/validate.js', upward(ROOT, path.join('core', 'mfg', 'validate.js'))]];
...
/* A1b — there must be NO second copy of the toolkit on disk. mfg-cap used to
   vendor core/interval/interval.js so it could stand alone; it no longer stands
...
{
  /* THE BUILDER'S OUTPUT PATH IS ASKED FOR, NOT ASSUMED. This read `path.join(ROOT,
     'mfg-cap.html')`, which was the builder's destination until 2026-08-08 — when the artifact
     moved to its route and the builder moved with it. The comment above already says "this
     battery does not get to choose it"; hard-coding the path is choosing it, quietly, and one
     move later the freshness check fails with a builder error instead of a verdict.
     build-artifact.js exports `dest` when imported (its write is guarded by require.main), so
     the battery imports it and reads the answer. One definition of where the artifact goes. */
  const BUILD_OUT = require(path.join(ROOT, 'tools', 'build-artifact.js')).dest;
   ...
   certified result, comparing against the kernels loaded directly. */
{
  ...
  if (carriers.length !== 1 || carriers[0].n !== 1) {
    ...
    if (MFG) {
      const M = require(upward(ROOT, path.join('core', 'mfg', 'mfg1d.js')));
      const V = require(upward(ROOT, path.join('core', 'mfg', 'validate.js')));
