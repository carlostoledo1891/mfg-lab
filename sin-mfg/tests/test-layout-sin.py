...
# monorepo path is kept, so a missing subject is still reported rather than silently substituted.
_SIN_CANDIDATES = [
    HERE.parent / 'sin-mfg.html',                            # monorepo
    # technical-reports/ ADDED 2026-07-30 — see the note in test-sin.js; the rename moved the
    # exported artifact and this list was not updated with it.
    HERE.parent.parent / 'technical-reports' / 'stock-constraint.html',  # export
    HERE.parent.parent / 'papers' / 'stock-constraint.html',             # export, pre-rename
]
TARGET = sys.argv[1] if len(sys.argv) > 1 else str(
    next((p for p in _SIN_CANDIDATES if p.exists()), _SIN_CANDIDATES[0]))
