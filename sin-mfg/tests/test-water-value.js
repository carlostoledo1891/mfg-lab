...
/* random tree sweep */
{
  ...
  let worstSigned = Infinity, signBad = 0;
  for (let k = 0; k < 120; k++) {
    ...
    n++;
    /* THE SIGN, measured on EVERY instance whether or not it certifies. Weak duality gives
       D >= rev for any dual-feasible w and any primal-feasible h, so signedGap < 0 is not a
       tolerance question — it is impossible, and it means this kernel's dual assembly is
       wrong. The old certificate reported Math.abs(rev - D) and could not distinguish the
       two directions at all. Deliberately OUTSIDE the treeOK() guard: a sign violation on a
       non-certifying instance is exactly the case a pass-filtered statistic would hide. */
    if (r.cert.signedGap < worstSigned) worstSigned = r.cert.signedGap;
    /* 1e-13, not the magnitude check's 1e-12: measured round-off on this sweep is 4.44e-16,
       so this leaves three orders of headroom AND sits BELOW the gapRel tolerance — which is
       the point. A negative gap of 5e-13 passes `gapRel < 1e-12` and is still impossible. */
    if (r.cert.signViolation > 1e-13) signBad++;
    if (treeOK(r.cert)) {
      certified++;
      worstGap = Math.max(worstGap, r.cert.gapRel);
  ...
  check('S3b weak duality holds with the right SIGN on every instance (D >= rev, not |D - rev|)',
    signBad === 0 && worstSigned >= -1e-12,
    'worst signedGap ' + worstSigned.toExponential(2) + ' over ' + n +
    ' trees, ' + signBad + ' sign violation(s) — negative is structurally impossible, not a tolerance');
