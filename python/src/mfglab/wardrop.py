...
def interior_start(pop: Pop, rng=None) -> np.ndarray:
    ...
    batteries; passing a numpy Generator (or any 0-arg callable returning a
    float in [0,1), e.g. mulberry32 for bit-parity with the JS battery)
    randomizes the split (weights 0.2+U).
    """
    draw = None if rng is None else (rng.random if hasattr(rng, "random") else rng)
    ...
    for u in TOPO:
        ...
        if not outs:
            continue
        w = [(0.2 + draw()) if draw is not None else 1.0 for _ in outs]
        sw = sum(w)
...
# Ported verbatim from test-wardrop.js: per population,
#   theta <- Proj_{K theta = B, theta >= 0}(theta - eta * c),
# projection by active-set pinning (equality-project with pinned coords at
# zero, pin new negatives, repeat). Exactness is CHECKED by the battery's
# 30 random VI tests, not assumed. This is the honest baseline for the
# geometric contrast the HRF paper argues: same certificates, different
# dynamics ("feasibility by geometry vs feasibility by repair").

def mulberry32(seed: int):
    """The artifact's RNG, ported bit-for-bit (uint32 arithmetic; the only
    float op is x/2^32, exact in both languages). Same seed -> the SAME
    stream as the JS battery, so reseed receipts are comparable across
    languages. Parity is gate-checked against a Node run in
    tests/test_crosslang_wardrop_battery.py."""
    s = seed & 0xFFFFFFFF

    def rng() -> float:
        nonlocal s
        s = (s + 0x6D2B79F5) & 0xFFFFFFFF
        t = ((s ^ (s >> 15)) * ((s | 1) & 0xFFFFFFFF)) & 0xFFFFFFFF
        # JS: t = (t + Math.imul(t ^ t>>>7, 61|t)) ^ t   (+ binds before ^)
        t = (((t + (((t ^ (t >> 7)) * ((t | 61) & 0xFFFFFFFF)) & 0xFFFFFFFF))
              & 0xFFFFFFFF) ^ t) & 0xFFFFFFFF
        return ((t ^ (t >> 14)) & 0xFFFFFFFF) / 4294967296.0

    return rng


def proj_pop(P: Pop, y) -> np.ndarray:
    """Exact polyhedral projection onto {K theta = B, theta >= 0} by
    active-set pinning, ported from projPop in test-wardrop.js (same rounds,
    same 1e-12 ridge, same worst-negative pin rule)."""
    n, m = P.n, P.m
    ...
    for _round in range(n + 2):
        ...
        Kf = P.K[:, free]
        rhs = Kf @ np.asarray(y)[free] - P.B
        M = Kf @ Kf.T + 1e-12 * np.eye(m)
        lam = gsolve(M, rhs)
        x.fill(0.0)
        worst, wv = -1, -1e-9
        for i in free:
...
def pg_run(sys: System, th1, th2, eta: float, max_steps: int, tol: float) -> dict:
    """Projected-gradient dynamics, ported from pgRun in test-wardrop.js.
    NEVER rig this comparison (house rule): PG is competitive in step count
    on this instance; the differentiator is structural (pre-projection
    Kirchhoff violation vs the flow's identical zero)."""
    c = np.zeros(NE)
    hist = []
    g = wardrop_gap(sys, th1, th2)
    steps = 0
    ...
    while steps < max_steps and g > tol:
        ...
        y1 = th1 - eta * c[np.asarray(sys.P1.act)]
        sys.cost(sys.J1, sys.J2, 2, c)
        y2 = th2 - eta * c[np.asarray(sys.P2.act)]
        th1[:] = proj_pop(sys.P1, y1)
        th2[:] = proj_pop(sys.P2, y2)
        steps += 1
        g = wardrop_gap(sys, th1, th2)
        if steps % 5 == 0 or g < tol:
            hist.append(g)
    return {"steps": steps, "gap": g, "hist": hist}
